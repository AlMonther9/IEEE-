...
To maintain consistency and readability, follow these file naming rules:

1. **Components**: Use PascalCase for all component files and directories. Split components into two categories:
   - **UI Components**: Place reusable UI elements under `/components/ui/`.
     - Example: `/components/ui/Button/Button.tsx`
   - **Other Components**: Place project-specific components directly under `/components/`.
     - Example: `/components/Header/Header.tsx`

2. **App Directory**: Use kebab-case for files in the `app` directory.
   - Example: `/app/not-found.tsx`

3. **Hooks**: Use camelCase and prefix with `use` for hook files.
   - Example: `/hooks/useSearch.tsx`

4. **Providers**: Use camelCase and include `Provider` in filenames.
   - Example: `/providers/authProvider.tsx`
...
These rules are enforced by ESLint using the `eslint-plugin-validate-filename`.
